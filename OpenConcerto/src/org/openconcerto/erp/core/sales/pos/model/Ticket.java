...
public class Ticket {
                    ...
                    try {
                        SQLUtils.executeAtomic(ds, new SQLUtils.SQLFactory<Object>() {
                            ...
                            public Object create() throws SQLException {
                                ...
                                rowValsTransact.put("ID_CLIENT", client.getId());
                                ...
                                int idMvt = ecr.getNewMouvement(table.getName(), rowTransact.getID(), 1, "Transact. " + client.getFullName() + " Ticket " + getCode());
                                ...
                                SQLRow row = tableClient.getRow(client.getId());
                                ...
                                new GenerationMvtVirement(idCompteAvanceClient, idCompteClient, 0, montantInCents, "Utilisation compte client", c.getTime(), JournalSQLElement.VENTES,
                                        "Ticket N°" + getCode()).genereMouvement();
                                return null;
                            }
                        });
    ...
    public void print(TicketPrinter prt, int ticketWidth) {
        ...
        for (SQLRowAccessor row : mapHtTVARowTaux.keySet()) {
            ...
            float tvaTaux = TaxeCache.getCache().getTauxFromId(row.getID());
    ...
    public void addArticle(Article a) {
        ...
        if (!alreadyExist) {
            Pair<Article, Integer> line = new Pair<Article, Integer>(new Article(a), 1);
            this.items.add(line);
        }
                ...
                try {
                    SQLUtils.executeAtomic(ds, new SQLUtils.SQLFactory<Object>() {
                        ...
                        public Object create() throws SQLException {
                            ...
                            SQLRow rowTransact = rowValsTransact.commit();
                            GenerationEcritures ecr = new GenerationEcritures();
                            int idMvt = ecr.getNewMouvement(table.getName(), rowTransact.getID(), 1, "Annule Transact. " + client.getFullName() + " Ticket " + getCode());
                            rowTransact = rowTransact.createEmptyUpdateRow().put("ID_MOUVEMENT", idMvt).commit();
                            ...
                            new GenerationMvtVirement(idCompteAvanceClient, idCompteClient, (long) montantInCents, 0L, "Annulation transaction compte client", getCreationCal().getTime(),
